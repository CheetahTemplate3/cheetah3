...
\item  Placeholder tags:  $placeholderName . $a.b.c.d  looks up attributes or
      keys.  Case sensitivity.  Trailing periods are not part of the 
      placeholder.  You can always use ${placeholderName} to make explicit 
      where the Placeholder Name ends.  Functions and methods will probably
      be autocalled by default; other callable types will never be autocalled
      (provided you don't fool the TO).  The caching rules are "stable unless
      something comes up".

\item  There will be a syntax for fuction arguments in () and 
      keys/subscripts in [].  It will be similar to the current syntax, 
      although we may have to make some adjustments.
