...
\item{Cheetah.Servlet.TemplateServlet}  ``An asbstract base class for Cheetah
     ...
     There is an \code{.initializeTemplate()} method you can override to do
     setup before the template is compiled.  This class also inherits from
     \code{WebKit.HTTPServlet.HTTPServlet} if Webware is available.  (If not,
     it fakes it with an empty class called HTTPServlet.)
...
\item{Cheetah.Templates.SkeletonPage}  A TemplateServlet subclass suitable as
     ...
     page header/footer, sidebars, etc, features for stylesheed dictionaries,
