...
Multiple inheritance is allowed, provided that the last base class is a
...
{\em Note:} In Cheetah 0.9.12 and earler, one listed the \code{Template} subclass
first rather than last.  The order was changed to allow mixin classes to override
standard methods like \code{.awake()}.  One caveat is that now it's possible to
accidentally override methods you weren't aware of.  Section ?? (not written yet)
lists all the standard methods \code{Template} objects have.
...
have an \code{\#extends} directive.  If the top of the hierarchy is a Python
class, it would have to subclass \code{Cheetah.Template.Template} explicitly.
