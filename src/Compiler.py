#!/usr/bin/env python
# $Id: Compiler.py,v 1.16 2001/11/06 22:27:32 tavis_rudd Exp $
...
Last Revision Date: $Date: 2001/11/06 22:27:32 $
"""
...
class ClassCompiler(SettingsManager, GenUtils):
    ...
    def __init__(self, className, mainMethodName='respond',
        ...
        if fileName and self.setting('monitorSrcFile'):
            ...
            self.addChunk('write(self._mainCheetahMethod_for_' + self._className 
                          + '(trans=trans))')            
            self.addStop()
            self.dedent()
