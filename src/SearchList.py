#!/usr/bin/env python
# $Id: SearchList.py,v 1.4 2001/08/08 23:38:41 tavis_rudd Exp $
...
class SearchList(UserList):
    ...
    def get(self, name, executeCallables=False):
        """Get the value for a $placeholder name"""
        for namespace in self.data:
            try:
                val = valueForName(namespace, name, executeCallables=executeCallables)
                return val
            except NotFound:
                pass           
        raise NotFound(name)
